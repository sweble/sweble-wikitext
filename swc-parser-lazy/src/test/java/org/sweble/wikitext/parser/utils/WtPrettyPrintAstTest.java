...
public class WtPrettyPrintAstTest
{
			...
			TestResourcesFixture resources) throws IOException, ParseException
	{
		...
		Object originalAst = test.parse(inputFile, visitors);
		
		String ppOriginalAst = test.printToString(originalAst, new TypedPrettyPrinter());
		...
		Object cleanedOriginalAst = test.parse(inputFile, cleaningVisitors);
		
		// -- print and compare ASTs
		
		TypedWtAstPrinter astPrinter = new TypedWtAstPrinter();
		
		String reparsedAstPrinted = test.printToString(cleanedReparsedAst, astPrinter);
		...
		{
			String originalAstPrinted = test.printToString(cleanedOriginalAst, astPrinter);

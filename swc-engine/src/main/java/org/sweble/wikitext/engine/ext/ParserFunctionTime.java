			...
			ParserFunctionBase
{
			...
			List<? extends AstNode> args)
	{
		...
		String format = expandArgToString(preprocessorFrame, args, 0);
		if (format == null)
		...
		if (args.size() >= 2)
		{
			timestampStr = expandArgToString(preprocessorFrame, args, 1);
		...
		if (args.size() >= 3)
		{
			languageTag = expandArgToString(preprocessorFrame, args, 2);
			if (languageTag == null)
	...
	private NodeList error(String msg)
	{
		return astList(
				astText("<strong class=\"error\">"),
				astProtectedText(StringUtils.escHtml(msg)),
				astText("</strong>"));
	}
	
	private AstNode notYetImplemented(String msg)
	{
		return astList(
				astText("<strong class=\"error not-yet-implemented\">"),
				astProtectedText(StringUtils.escHtml(msg)),
				astText("</strong>"));
	}
}

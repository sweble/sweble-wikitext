			...
			PtkAstVisitor
{
	...
	protected Object resolveAndVisit(AstNode n, int type) throws ExpansionException
	{
		switch (type)
		{
			...
			case AstNodeTypes.NT_MAGIC_WORD:
				return visit((MagicWord) n);
			default:
	...
	private AstNode visit(Template n) throws ExpansionException
	{
		...
		AstNode result = resolveTemplateAsPfn(n, title, tail, args);
		...
		if (result == null)
		{
			...
			if (tail == null)
			{
				...
				if (args.isEmpty())
					result = resolveTemplateAsMagicWord(n, title);
				
				// If not try to transclude
				if (result == null)
					result = resolveTemplateAsTransclusion(n, title, args);
			}
			else
			{
	...
	private AstNode resolveTemplateAsPfn(
			...
			String title,
			NodeList tail,
			ArrayList<TemplateArgument> args) throws ExpansionException
	{
		int i = title.indexOf(':');
		if (i == -1)
			return null;
		...
		String arg0Prefix = title.substring(i + 1);
		...
		return invokePfn(n, pfn, argsValues);
	}
	 ...
	 *         error occurs afterwards, the template node itself is returned.
	 */
	private AstNode resolveTemplateAsMagicWord(Template n, String title) throws ExpansionException
	{
	...
	private AstNode invokePfn(
			...
			List<? extends AstNode> argsValues) throws ExpansionException
	{
		if (hooks != null)
		{
		...
		if (frameLog != null)
		{
			log = new ResolveParserFunctionLog(pfn.getName(), false);
			frameLog.getContent().add(log);
		}
		...
		try
		{
			...
			if (result == null)
				throw new NullPointerException("Parser function `" + pfn.getName() + "' returned null value!");
			...
			ResolveTransclusionLog log)
	{
		...
		for (TemplateArgument arg : args)
		{
			...
			String id = String.valueOf(transclArgs.size() + 1);
			
			transclArgs.put(id, value);
			...
			if (arg.getHasName())
			{
				...
				try
				{
					String nameStr = StringConverter.convert(name);
					
					nameStr = nameStr.trim();
					
					if (!nameStr.isEmpty() && !nameStr.equals(id))
	...
	{
		return resolveTagExtensionWrapper(n, n.getName(), n.getXmlAttributes(), n.getBody());
	}
	...
	private AstNode visit(MagicWord n) throws ExpansionException
	{
		return resolveMagicWordWrapper(n, n.getWord());
	}
			...
			String name) throws ExpansionException
	{
		if (hooks != null)
		{
			AstNode cont = hooks.beforeResolveMagicWord(this, n, name);
			if (cont != ExpansionDebugHooks.PROCEED)
				return cont;
		}
		...
		try
		{
			result = resolveMagicWord(n, name, log);
		}
		catch (Exception e)
		{
			result = n;
		...
		return (hooks != null) ?
				hooks.afterResolveMagicWord(this, n, name, result, log) :
				result;
	}
	
	private AstNode resolveMagicWord(
			MagicWord n,
			String name,
			ResolveMagicWordLog log)
	{
		MagicWordBase mw =
				getWikiConfig().getMagicWord(name);
		...
		AstNode result = mw.invoke(expFrame, n);
	...
	private WikiConfigurationInterface getWikiConfig()
	{
